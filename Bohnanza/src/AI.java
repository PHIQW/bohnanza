...
public class AI {
	...
	/*The idea for this AI is you trade so that your hand is ready-to-plant for next turn
	 * This can be achieved by:
	 * 		-trading away cards in your current hand so that the first 2 cards in your hand can be put into your fields
	 *		-get cards from trades so that your field(s) are set up for your hand next turn
	 */	

	private ArrayList<Card> tempOffer = new ArrayList<Card>();
	private ArrayList<Card> cOffer = new ArrayList<Card>();
	private double[] baseValue = {};
	private static int[] defaultProb = {20,18,16,14,12,10,8,6};//hard coded from csv at start
	private int[] publicProb = new int[8];
	private static String[] beanName = {"Blue Bean","Chili Bean","Stink Bean","Green Bean","Soy Bean","Black-eyed Bean","Red Bean","Garden Bean"};
	...
	//decide between 2 filled fields
	public static int decField(Player u){
		
		int[] potential = {0,0};

		//plant on field with less potential(probablity to get card)
		for(int i=0;i<u.getField().size();i++)
			for(int k=0;k<beanName.length;k++)
				if(u.getField().get(i).get(0).getName() == beanName[k])
					potential[i]+=defaultProb[k];
	...
	public void createOffer(Player p){
		...
		while(value < 2 && tempHand.size()<=3){
			...
			//try to set up hand with trade for next turn	b/c you want the first 2 cards in hand to be playable next turn
			for(int i=0;i<p.getField().size();i++)
				for(int x=0;x<2;x++)//2 b/c you want first 2 cards to be playable
					if(p.getField().get(i).size()>0 && p.getHand().size()>=2+x && tempHand.size()<3){//if there are cards in field
						if(tempHand.get(x) != p.getField().get(i).get(0) && tempHand.get(x+1) == p.getField().get(i).get(0)){
	...
	public void counterOffer(Player p, Player o){
		...
		//if you have empty fields, make offer more valuable to other player by offering cards in their field
	...
	public boolean checkDealC(Player p){//counteroffer
		...
		//field is developed enough and cards offer has cards in your first 2 slots in your hand
		for(int h=0;h<2;h++)
			for(int i=0;i<tempOffer.size();i++)
				if(p.getHand().get(h) == tempOffer.get(i))
					val++;

		if(val>=tempOffer.size()-1 && val>=1)
			return true;
	...
	private double value(ArrayList<Card> huff){

		double val = 0;

		for(int i=0;i<huff.size();i++){
			for(int k=0;k<beanName.length;k++)
				if(huff.get(i).getName()==beanName[k])
					val+=Math.sqrt(k);
		}
